...
public class CollectionUtil {
	 ...
	 * @return object
	 */
	public static <T> T findListElement(List<T> list, String key, String value) {

		if (CollectionUtil.isEmptyCollection(list) || CommonUtil.isEmpty(key)
				|| CommonUtil.isEmpty(value))
			return null;
		for (T t : list) {
			try {
				if (value.equals(BeanUtils.getProperty(t, key))) {
					return t;
				}
	...
	/**
	 * 将指定的字符串用指定的分隔符分隔，并映射为指定的java对象，加入到集合中
	 * 
	 * @param str
	 * @param regex
	 * @param set
	 * @param clazz
	 * @return Collection<T> or null
	 */
	public static <T> Collection<T> splitStrToCollection(String str, String regex,
			Collection<T> set, Class<T> clazz) {

		if (CommonUtil.isEmpty(str) || regex == null || set == null)
			return null;
		...
		if (array != null) {
			T t = null;
			for (String s : array) {
				t = (T) ConvertUtils.convert(s.trim(), clazz);
				set.add(t);
			}
		}
	 ...
	 * @param to
	 * @param isUpper
	 *            if isUpper is true,the value converted to uppercase and then copy
	 */
	public static <T extends Object> void copyMap(Map<String, T> from, Map<String, Object> to,
			boolean isUpper) {
